...
compatibility gate + dependency-graph + hidden-access audit, and real
end-to-end functional tests. Every step runs as a real subprocess against
real files this script creates fresh under verification/ (library_build/,
work/, fixtures/, functest_scratch/) -- never against OUTPUT_LIBRARY or
...
def main():
    ...
    for d in ("library_build", "work", "fixtures", "gen_prove", "functest_scratch"):
        p = HERE / d
        if p.exists():
            shutil.rmtree(p)
    ...
    section("1/6  Internal regression suite (verify_build.py + test_readiness.py)")
    ...
    section("2/6  All 43 canonical apps (build_batch.py)")
    ...
    section("3/6  3 new composed apps")
    new_apps = {}
    ...
    section("4/6  Generalized-engine regression (prove_generalization.py)")
    ...
    section("5/6  Compatibility gate + dependency-graph + hidden-access audit")
    ...
    section("6/6  End-to-end functional tests (functional_tests.py)")
