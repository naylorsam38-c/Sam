...
Reason: SPEC_AUTHORITY_DECISION.md found this document stale in two named places (§C.4's data-collision gap claim, §C.6's journey-auth gap claim) relative to the real, current, verified implementation. God_Mode_Specification.md carries forward every rule that passed alignment, corrects these two, and adds the two real mechanisms this document never documented at all.
Original path: `CANONICAL_SPEC.md` (repo root)
Archived: 2026-09-13
...
**Components governed.** `_namespace_route()` (route strings), the same-app `(route, method)` collision assertion in `add_capability()`, the `RESERVED_SHARED_LIB_FILENAMES` generation-time guard (added in this contract-verification pass), and every capability's own choice of `DATA_FILE_NAME`.
...
- **`RESERVED_SHARED_LIB_FILENAMES` (`verification/gen_common.py`, module-level constant: `frozenset({"auth_sessions.json", "blobs.json"})`), enforced inside `add_capability()`** immediately after `df = data_filename or self.data_filename`: raises `AssertionError` if any capability's resolved data filename equals one of these two names. These are the two filenames used exclusively as CAP-0000's own internal primitive state (`create_session()`/`validate_session()`/`invalidate_session()`'s session store; `save_blob()`/`load_blob()`'s default blob index) and — verified by grepping every `data_filename=` call site in `gen_common.py` before adding the guard — never legitimately passed as any capability's own `data_filename` by any of the 20 generic engines. `"api_keys.json"` and `"share_tokens.json"` are deliberately **not** reserved: those two names ARE a capability's own legitimate `data_filename`, declared by their owning engine (`add_api_key_capability`/`add_share_token_capability`), and reserving them would have broken real, working capabilities.

**Test evidence.** Two real, historical violations, both found by this exact mechanism and both fixed; a third protection now verified in place:
...
3. **`RESERVED_SHARED_LIB_FILENAMES` guard, added and verified in this contract-verification pass**: a targeted unit test confirmed the guard raises `AssertionError` for both `"auth_sessions.json"` and `"blobs.json"` when passed as a capability's `data_filename`, and confirmed `add_share_token_capability()`/`add_api_key_capability()` (the two engines that legitimately use adjacent reserved-*looking* names) still build with no error. Full regression re-run afterward, live: proving table 29/29, canonical apps 43/43 READY, composed apps 6/6, dependency-graph audit CLEAN (294 capabilities / 49 projects), functional tests 30/30, isolated stress test 184/184 (0 shadowed, 0 other-fail). All 15 coverage-expansion apps rebuilt fresh; `security_tests.py` 45/45, `encryption_tests.py` 9/9, `document_tests.py` 12/12, `ranking_tests.py` 9/9 — all unchanged from the pre-guard baseline.

Final state, live-reconfirmed after the guard: **323/323** capabilities across the full merged library (canonical + all 15 coverage-expansion apps), **0** collisions, **0** shadowed, **0** other failures, 0 non-byte-identical duplicate ids.

**Proposed new requirements.** None that would change tested behavior.

**Genuine gaps — updated, not closed.** Bug #1 (routes) has a real, structural, generation-time guard that makes recurrence impossible. Bug #2's *specific* recurrence (a domain capability colliding with a CAP-0000-owned primitive filename) now also has one: the `RESERVED_SHARED_LIB_FILENAMES` guard above. **What remains genuinely open**: this guard only covers collisions against CAP-0000's own two reserved names. It does **not** protect against two arbitrary *domain* capabilities in different apps independently choosing the same data filename for unrelated purposes — the original, narrower form bug #2 actually took (before the auth engine existed, `course_enrollment_hub`'s and `multiplayer_game`'s own, mutually-unrelated capabilities already both used `sessions.json`, and still do — confirmed present, harmless only because neither's schema currently crashes the other's code path, not because anything prevents it). Catching *that* broader class still depends on running `full_library_stress_test.py` with the specific combination of apps involved, exactly as it did before this pass. This narrower, residual gap is recorded as the remaining "required future improvement," honestly scoped down from the guard just added rather than claimed as fully closed.
