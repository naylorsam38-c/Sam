...
`SPEC-FRONTDOOR` — assembled from `booking-frontdesk+accounting-ledger` against graph 3.0. Every field below is numbered by the question, default, or derivation that owns it — nothing here was guessed.
...
| B.03 | `billing.plans` | [{"name": "Per appointment", "price": "the Price set on the Service being booked… | question |
...
| # | Field | Value | Source |
|---|---|---|---|
| C.06 | `client.landing_screen_per_role` | {"Owner": "Calendar", "Staff": "Calendar", "Admin": "Invoices list", "Accountant… | question |
| C.05 | `client.mobile_behaviour` | {"mode": "simplified", "what": "the public booking flow is mobile-first: one ste… | question |
...
| A.15 | `inventory.notifications` | ["Booking confirmation", "Appointment reminder", "Cancellation notice", "Invoice… | question |
| A.15 | `inventory.records` | ["Service", "Contact", "Appointment", "Invoice", "Invoice line", "Bill", "Paymen… | question |
| A.15 | `inventory.reports` | ["Upcoming appointments", "No-show rate"] | question |
| A.15 | `inventory.roles` | ["Owner", "Staff", "Admin", "Accountant", "Advisor"] | question |
| A.15 | `inventory.screens` | [] | question |
| A.15 | `inventory.workflows` | ["Appointment lifecycle", "Invoice lifecycle", "Bill lifecycle"] | question |
...
| # | Field | Value | Source |
|---|---|---|---|
| A.03 | `product.audience` | Your team's roles are kept, and outsiders get a public page with no account. | question |
| A.01 | `product.description` | Take bookings for my clinic and invoice people afterwards. | question |
| A.02 | `product.goals` | Use it to take bookings for my clinic and invoice people afterwards. | question |
| A.05 | `product.name` | Front Room | question |
...
| P.04:Admin | `role.Admin.assignable_by` | ["Owner"] | question |
| P.03:Admin | `role.Admin.billing_access` | no | question |
| P.01:Admin | `role.Admin.description` | Was in charge before the pieces were joined; now an ordinary role, with the same… | question |
| P.02:Admin | `role.Admin.sees_private_data` | no | question |
...
| C.04 | `visual.brand_assets` | {"mode": "premade", "logo_id": "wave"} | question |
| C.03 | `visual.density` | spacious | question |
...
| C.02 | `visual.tone` | ["quick", "simple", "direct"] | question |
...
- Records: Service, Contact, Appointment, Invoice, Invoice line, Bill, Payment
- Roles: Owner, Staff, Admin, Accountant, Advisor (super: Owner)
- Workflows: Appointment lifecycle, Invoice lifecycle, Bill lifecycle
- Screens: 19 (`SPEC-FRONTDOOR` navigation order)
- Actions: 38
- Generated QA tests: 86
