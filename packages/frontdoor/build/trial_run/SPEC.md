...
`SPEC-FRONTDOOR` — assembled from `pm-teamwork` against graph 3.0. Every field below is numbered by the question, default, or derivation that owns it — nothing here was guessed.
...
| A.01 | `product.description` | Run my small team's projects: tasks, who's doing what, and whether it's actually… | question |
| A.02 | `product.goals` | Use it to run my small team's projects: tasks, who's doing what, and whether it'… | question |
| A.05 | `product.name` | Teamwork Desk | question |
...
| C.04 | `visual.brand_assets` | {"mode": "premade", "logo_id": "grid"} | question |
...
| C.02 | `visual.tone` | ["clear", "efficient", "calm"] | question |
...
- Screens: 8 (`SPEC-FRONTDOOR` navigation order)
- Actions: 14
