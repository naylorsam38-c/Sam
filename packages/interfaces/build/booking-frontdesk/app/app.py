#!/usr/bin/env python3
"""GENERATED by packages/builder/builder.py from SPEC-BOOKING-FRONTDESK-REF (Front Desk (booking-frontdesk)).
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET(self):
        ...
        if path == '/api/customers':
            self.list_customers(); return
        if path.startswith('/api/customers/'):
            self.get_customers(path[len('/api/customers/'):]); return
        if path.startswith('/api/history/customers/'):
            self.history_customers(path[len('/api/history/customers/'):]); return
    ...
    def _do_POST(self):
        ...
        if path == '/api/customers':
            self.create_customers(self._body()); return
        if path == '/api/appointments':
    ...
    def _do_PUT(self):
        ...
        if path.startswith('/api/customers/'):
            self.update_customers(path[len('/api/customers/'):], self._body()); return
        if path.startswith('/api/appointments/'):
    ...
    def _do_DELETE(self):
        ...
        if path.startswith('/api/customers/'):
            self.delete_customers(path[len('/api/customers/'):]); return
        if path.startswith('/api/appointments/'):
    ...
    def get_customers(self, record_id):
        rows = query("SELECT * FROM customers WHERE id = ?", (record_id,))
        if not rows:
            respond(self, 404, {"error": "not found"}); return
        respond(self, 200, rows[0])

    def history_customers(self, row_id):
        """The row's own real trail, oldest first."""
        conn = get_db()
        try:
            trail = audit_trail.history_for(conn, "customers", row_id)
            stages = [dict(r) for r in conn.execute(
                "SELECT stage, entered_at FROM _stage_history WHERE record_table = ? AND row_id = ? ORDER BY entered_at",
                ("customers", str(row_id))).fetchall()]
            respond(self, 200, {"audit": trail, "stages": stages})
        finally:
            conn.close()

    def create_customers(self, body):
        rid = new_id(); now = now_iso()
        values = [rid, now, now, body.get('Full name'), body.get('Email'), body.get('Phone'), body.get('Notes')]
        execute("INSERT INTO customers ([id], [created_at], [updated_at], [full_name], [email], [phone], [notes]) VALUES (?, ?, ?, ?, ?, ?, ?)", values)
        effects = []
        respond(self, 201, {"id": rid, "effects": effects})

    def update_customers(self, record_id, body):
        values = [body.get('Full name'), body.get('Email'), body.get('Phone'), body.get('Notes'), now_iso(), record_id]
        n = execute("UPDATE customers SET [full_name] = ?, [email] = ?, [phone] = ?, [notes] = ?, updated_at = ? WHERE id = ?", values)
        if not n:
            respond(self, 404, {"error": "not found"}); return
        respond(self, 200, {"id": record_id})

    def delete_customers(self, record_id):
        n = execute("DELETE FROM customers WHERE id = ?", (record_id,))
        respond(self, 200 if n else 404, {"deleted": bool(n)})
    ...
    def submit_public_booking_form(self, body):
        ...
        fields = [{"name": "Service", "type": "link", "required": "yes", "unique": "no", "target_record": "Service"}, {"name": "Customer", "type": "link", "required": "yes", "unique": "no", "target_record": "Customer"}, {"name": "Staff member", "type": "other", "required": "yes", "unique": "no", "custom_rule": "reference to a user account (pick-a-person field)"}, {"name": "Start", "type": "date_time", "required": "yes", "unique": "no"}, {"name": "Notes", "type": "long_text", "required": "no", "unique": "no"}, {"name": "Deposit paid", "type": "yes_no", "required": "no", "unique": "no"}]
        conn = get_db()
        try:
            try:
                now = now_iso()
