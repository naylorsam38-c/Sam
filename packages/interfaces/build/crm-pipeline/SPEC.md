...
`SPEC-CRM-PIPELINE-REF` — assembled from `crm-pipeline` against graph 3.0. Every field below is numbered by the question, default, or derivation that owns it — nothing here was guessed.
...
| # | Field | Value | Source |
|---|---|---|---|
| A.03 | `product.audience` | A sales team: reps who own deals and a manager who sees everything. | question |
| A.01 | `product.description` | A sales pipeline: organisations and contacts, deals moving through stages from L… | question |
| A.02 | `product.goals` | Track every deal from first contact to close, know the pipeline value by stage, … | question |
| A.05 | `product.name` | Pipeline | question |
| A.04 | `product.success_definition` | The pipeline is honest -- every deal is in the stage it is really in -- and the … | question |
...
| C.02 | `visual.tone` | ["focused", "energetic", "confident"] | question |
...
- Screens: 10 (`SPEC-CRM-PIPELINE-REF` navigation order)
- Actions: 21
