...
ROUTE = '/api/note_taking/auth/register'
METHOD = 'POST'


def _handle_inner(request):
    ...
    user = {'id': next_id, 'email': email, 'password_hash': _shared.hash_password(password), 'role': None}
    users.append(user)
    _save(users)
    _shared.audit(email, 'register', 'users.json', next_id, 'role=' + repr(None))
    return 201, {k: v for k, v in user.items() if k != 'password_hash'}


def handle(request, ctx):
    if ctx.get('role') != 'admin':
        return 403, {'error': 'requires role admin'}
