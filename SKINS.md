...
Nginx Proxy Manager, Memos, Galene, Mautic, Ghost, and langfuse** (the last on its final
...
per Mautic's own compatibility matrix; Ghost on Node 22.23.1, its own primary pinned version,
after a real Node-24-specific native-module crash in `better-sqlite3` surfaced on the
alternative version its own `engines` field also allows). Full per-app mechanism notes, real
